...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(AlphaVantageRateLimitException.class)
    public ResponseEntity<Map<String, Object>> handleRateLimit(AlphaVantageRateLimitException e) {
        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                .body(body(HttpStatus.SERVICE_UNAVAILABLE, e.getMessage()));
    }

    // Catch-all: log the full exception server-side but never expose internal
    // details (class names, stack traces, Redis connection strings) to the client.
    @ExceptionHandler(Exception.class)
    public ResponseEntity<Map<String, Object>> handleGeneric(Exception e) {
        log.error("Unhandled exception: {}", e.getMessage(), e);
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(body(HttpStatus.INTERNAL_SERVER_ERROR, "An unexpected error occurred. Please try again."));
    }
}
